...
static int convert_image_cpu(image_buffer_t *src, image_buffer_t *dst, image_rect_t *src_box, image_rect_t *dst_box, char color) {
    LOGI("==== convert_image_cpu ====");
    LOGI("cpu format: src=%d, dst=%d", src->format, dst->format);
    int ret;
    ...
    if (src->format == IMAGE_FORMAT_RGB888) {
        ...
        LOGI("IMAGE_FORMAT_RGB888: reti=%d", reti);
        ...
        LOGI("IMAGE_FORMAT_RGBA8888: reti=%d", reti);
...
static int convert_image_rga(image_buffer_t* src_img, image_buffer_t* dst_img, image_rect_t* src_box, image_rect_t* dst_box, char color)
{
    ...
    LOGI("use_handle=%d, src=%p, dst=%p, src_fd=%d, dst_fd=%d, srcFmt=0x%x, dstFmt=0x%x", use_handle, src, dst, src_fd, dst_fd, srcFmt, dstFmt);
    ...
    // rga process
    LOGI("src buffer: vir_addr=%p, width=%d, height=%d, format=0x%x, fd=%d",
         rga_buf_src.vir_addr, rga_buf_src.width, rga_buf_src.height, rga_buf_src.format, src_fd);

// 目标 buffer
    LOGI("dst buffer: vir_addr=%p, width=%d, height=%d, format=0x%x, fd=%d",
         rga_buf_dst.vir_addr, rga_buf_dst.width, rga_buf_dst.height, rga_buf_dst.format, dst_fd);

// 裁剪矩形
    LOGI("srect: x=%d, y=%d, w=%d, h=%d", srect.x, srect.y, srect.width, srect.height);
    LOGI("drect: x=%d, y=%d, w=%d, h=%d", drect.x, drect.y, drect.width, drect.height);
    LOGI("prect: x=%d, y=%d, w=%d, h=%d", prect.x, prect.y, prect.width, prect.height);

// usage
    LOGI("usage flags: 0x%x", usage);

// pat buffer (通常是全0)
    LOGI("pat buffer: vir_addr=%p, width=%d, height=%d, format=0x%x",
         pat.vir_addr, pat.width, pat.height, pat.format);
    ret_rga = improcess(rga_buf_src, rga_buf_dst, pat, srect, drect, prect, usage);
    LOGE("src: w=%d h=%d fmt=%d addr=%p\n", rga_buf_src.width, rga_buf_src.height, rga_buf_src.format, rga_buf_src.vir_addr);
    LOGE("dst: w=%d h=%d fmt=%d addr=%p\n", rga_buf_dst.width, rga_buf_dst.height, rga_buf_dst.format, rga_buf_dst.vir_addr);

    if (ret_rga <= 0) {
        LOGE("improcess ret=%d, err=%s", ret_rga, imStrError(ret_rga));
...
int convert_rgba_to_rgb(image_buffer_t* src, image_buffer_t* dst) {
    ...
    uint8_t* src_ptr = src->virt_addr;
    uint8_t* dst_ptr = dst->virt_addr;
...
//{
//    if (!src_image || !dst_image) return -1;
//
//    uint8_t* rgb_buffer = NULL;
//
//    // 1. 如果 src 是 RGBA，需要转换
//    if (src_image->format == IMAGE_FORMAT_RGBA8888 && dst_image->format == IMAGE_FORMAT_RGB888) {
//        int size = src_image->width * src_image->height * 3;
//        rgb_buffer = (uint8_t*)malloc(size);
//        if (!rgb_buffer) return -1;
//
//        for (int i = 0; i < src_image->width * src_image->height; i++) {
//            rgb_buffer[i*3 + 0] = src_image->virt_addr[i*4 + 0]; // R
//            rgb_buffer[i*3 + 1] = src_image->virt_addr[i*4 + 1]; // G
//            rgb_buffer[i*3 + 2] = src_image->virt_addr[i*4 + 2]; // B
//        }
//    } else {
//        // 如果本身就是 RGB，直接用 src
//        rgb_buffer = src_image->virt_addr;
//    }
//
...
//
//
//    // 2. 分配 dst_image buffer
//    if (!dst_image->virt_addr) {
//        dst_image->size = get_image_size(dst_image);
//        dst_image->virt_addr = (uint8_t*)malloc(dst_image->size);
//        if (!dst_image->virt_addr && rgb_buffer != src_image->virt_addr) {
//            free(rgb_buffer);
//            return -1;
//        }
//    }
...
//    tmp_src.width = src_image->width;
//    tmp_src.height = src_image->height;
//    tmp_src.format = IMAGE_FORMAT_RGB888; // 必须和 dst 一致
//    tmp_src.virt_addr = rgb_buffer;
//    tmp_src.size = src_image->width * src_image->height * 3;
//
//    int ret = convert_image(&tmp_src, dst_image, &src_box, &dst_box, color);
//    // 这里 convert_image 函数使用 rgb_buffer 作为 src
////    int ret = convert_image_buffer(rgb_buffer, dst_image, src_image->width, src_image->height,
////                                   dst_image->width, dst_image->height, letterbox, color);
//
//    // 4. 如果 malloc 了 rgb_buffer，需要在使用完 dst_image 后 free
//    if (rgb_buffer != src_image->virt_addr) {
//        free(rgb_buffer);
//    }
//
//    return ret;
...
int convert_image_with_letterbox(image_buffer_t* src_image, image_buffer_t* dst_image, letterbox_t* letterbox, char color)
{
    ...
    LOGI("==== convert_image_with_letterbox ====");
...
//    }
    ...
    ret = convert_image(&tmp_src, dst_image, &src_box, &dst_box, color);
    // 这里 convert_image 函数使用 rgb_buffer 作为 src
//    int ret = convert_image_buffer(rgb_buffer, dst_image, src_image->width, src_image->height,
//                                   dst_image->width, dst_image->height, letterbox, color);
