...
//    src_image.virt_addr = static_cast<unsigned char *>(dstBuf);
    src_image.virt_addr = (uint8_t*)dstBuf; // 仅读
    src_image.size = dstInfo.width * dstInfo.height * 4;

    LOGI("IMAGE_FORMAT_RGBA8888=%d", IMAGE_FORMAT_RGBA8888);
    LOGI("IMAGE_FORMAT_RGB888=%d", IMAGE_FORMAT_RGB888);
...
//    }
    ...
    // 打印检测到的目标数量
    LOGI("Detected object count: %d", od_results.count);
    ...
    for (int i = 0; i < od_results.count; i++) {
        ...
        // 打印每个目标的信息
//        LOGI("Object %d: %s [%.3f] @ (%d, %d, %d, %d)", i, cls_name, prob, x1, y1, x2, y2);
